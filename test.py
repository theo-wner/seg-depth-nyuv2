...
if __name__ == '__main__':
    ...
    cnt = 0
    for image, label, depth, seg_preds, depth_preds in predict(model, 100, config.TASK):
        if config.TASK == 'seg':
            visualize_img_label(image.cpu(), label.cpu(), seg_preds.cpu(), filename=f'seg_{cnt}.png')
        elif config.TASK == 'depth':
            visualize_img_depth(image.cpu(), depth.cpu(), depth_preds.cpu(), filename=f'depth_{cnt}.png')
        elif config.TASK == 'segdepth':
            visualize_img_label_depth(image.cpu(), label.cpu(), seg_preds.cpu(), depth.cpu(), depth_preds.cpu(), filename=f'segdepth_{cnt}.png')
